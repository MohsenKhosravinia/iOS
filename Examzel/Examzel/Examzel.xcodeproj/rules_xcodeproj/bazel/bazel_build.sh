...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@rules_xcodeproj_generated//generator/Examzel/Examzel_xcodeproj:Examzel_xcodeproj" \
...
if [[ -n "${target_ids:-}" ]]; then
  diff_output=$(comm -23 <(printf '%s\n' "${target_ids[@]}") "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/Examzel/Examzel_xcodeproj/Examzel_xcodeproj_target_ids")
